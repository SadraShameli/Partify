...
import { IUserRegistrationForm } from './UserTypes';
...
export default function UserRegistrations() {
    const {
        ...
        handleSubmit,
        formState: { errors },
    } = useForm<IUserRegistrationForm>({ reValidateMode: 'onChange' });

    function onSubmit(data: IUserRegistrationForm) {
        console.log(data);
    }

    return (
        <div className='grid place-items-center'>
            <form className='w-full max-w-lg mt-hero' onSubmit={handleSubmit(onSubmit)}>
                <Card>
                    ...
                    <div className='grid gap-8'>
                        ...
                        <InputField
                            ...
                            placeholder='&#9679;&#9679;&#9679;&#9679;&#9679;&#9679;'
                            {...register('passwordConfirmation', { required: true, pattern: ValidationPatterns.password })}
                        >
                            {errors.passwordConfirmation?.type === 'required' && <InputInfoText>This field is required</InputInfoText>}
                            {errors.passwordConfirmation?.type === 'minLength' && <InputInfoText>Minimum of 8 characters</InputInfoText>}
                        </InputField>

                        <Checkbox title='I accept the'>
                            <Link className='text-primary pl-1' title='Terms and Conditions' type='button' href='login'>
                                Terms and Conditions.
                            </Link>
                        </Checkbox>

                        <button className='btn btn-primary' title='Create an account'>
                            Create an account
                        </button>
                        ...
                        <div className='flex gap-2'>
                            ...
                            <Link className='text-primary ' title='Sign in page' type='button' href='login'>
                                Sign in here
                            </Link>
                        </div>
                    </div>
