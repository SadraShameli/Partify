    ...
    return (
        <div className='flex pt-10'>
            <div className='relative w-48'>
                <Image className='img' src={props.Image} alt={props.Name} loading='lazy' fill />
            </div>
            ...
            <form className='flex-auto p-6'>
                ...
                <div className='pb-6 mt-4 mb-6'>
                    <div className='space-x-2'>
                        {props.Variants.map((variant, index) => {
                            return (
                                <label key={index}>
                                    <input className='peer sr-only' name='size' type='radio' value={variant.Title} {...(variant.InStock && { checked: true })} />
                                    <div className={`btn btn-sm peer-checked:btn-active btn-ghost ${variant.InStock || 'btn-disabled'}`}>{variant.Title}</div>
                                </label>
                            );
                        })}
                    </div>
                ...
                <div className='flex mb-6 space-x-4 text-sm font-medium'>
                    ...
                    <div className='w-9 h-9 place-items-center btn btn-sm btn-outline grid'>
